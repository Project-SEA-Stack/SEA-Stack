#!/usr/bin/env python3
"""
F3OF Decay C3 Regression Test Comparison Script

This script compares SEA-Stack results for F3OF Decay C3 (flap pitch decay test)
...
def main():
    # F3OF Decay C3 specific configuration
    test_name = "F3OF Decay C3 Flap Pitch Decay"
    executable_patterns = ["f3of_decay_c3_test", "f3of_decay_c3_test.exe"]
    ...
    try:
        ...
        test_configs = [
            {
                ...
                'status_name': 'f3of_decay_c3_flap_fore_pitch'
            },
            {
                ...
                'validation_tolerance': (1e-6, 1e-6),
                'status_name': 'f3of_decay_c3_flap_aft_pitch'
            }
        ]
        ...
        if diff_base_surge > 1e-6:
            print(f"F3OF Decay C3 validation failed: Base surge difference {diff_base_surge:.2e} > 1e-6")
            sys.exit(1)
        ...
        if diff_base_pitch > 1e-10:
            print(f"F3OF Decay C3 validation failed: Base pitch difference {diff_base_pitch:.2e} > 1e-10")
            sys.exit(1)
        ...
        if diff_flap_fore_pitch > 1e-6:
            print(f"F3OF Decay C3 validation failed: Flap fore pitch difference {diff_flap_fore_pitch:.2e} > 1e-6")
            sys.exit(1)
        ...
        if diff_flap_aft_pitch > 1e-6:
            print(f"F3OF Decay C3 validation failed: Flap aft pitch difference {diff_flap_aft_pitch:.2e} > 1e-6")
            sys.exit(1)
        ...
        if all_passed:
            print("F3OF Decay C3 TEST PASSED - All comparisons within tolerance")
            print(f"Generated plots:")
            for config in test_configs:
                plot_name = config['test_name'].lower().replace(' ', '_').replace('-', '_')
                print(f"  - {plots_dir}/{plot_name}_comparison.png")
        else:
            print("F3OF Decay C3 TEST FAILED - Some comparisons outside tolerance")
            sys.exit(1)
