...
int main(int argc, char* argv[]) {
    ...
    auto body1_meshfile = (DATADIR / "demos" / "f3of" / "geometry" / "base.obj").lexically_normal().generic_string();
    auto body2_meshfile = (DATADIR / "demos" / "f3of" / "geometry" / "flap.obj").lexically_normal().generic_string();
    ...
    double simulationDuration = seastack::chrono::GetSimDuration(100.0, 240.0);
    ...
    // Decay C2 set up (flaps locked, base pitch decay, no waves)
    ...
    HydroSystem hydroforces(bodies, h5fname, default_dont_add_waves);
