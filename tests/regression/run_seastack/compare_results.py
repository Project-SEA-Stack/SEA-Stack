...
def main() -> int:
    ...
    # Plot using the standard regression template (sibling compare_template.py in
    # run_seastack/ source and in flat packaged tests/).
    try:
        template_dir = Path(__file__).resolve().parent
        if not (template_dir / "compare_template.py").is_file():
            for cand in (template_dir.parent / "regression", template_dir.parents[1]):
                if (cand / "compare_template.py").is_file():
                    template_dir = cand
                    break
