...
def _resolve_tests_utilities() -> Path:
    """tests/utilities next to this file (CPack flat tests/) or nested source tree."""
    here = Path(__file__).resolve().parent
    flat = here / "utilities"
    if (flat / "plot_helpers.py").is_file():
        return flat
    nested = here.parent.parent.parent / "utilities"
    if (nested / "plot_helpers.py").is_file():
        return nested
    cur = here
    for _ in range(8):
        cand = cur / "utilities" / "plot_helpers.py"
        if cand.is_file():
            return cand.parent
        if cur == cur.parent:
            break
...
_TESTS_UTILITIES = _resolve_tests_utilities()
if str(_TESTS_UTILITIES) not in sys.path:
    sys.path.insert(0, str(_TESTS_UTILITIES))
from plot_helpers import get_chrono_version  # noqa: E402
...
def get_seastack_version():
    """Get SEA-Stack version from CMakeCache.txt"""
    try:
