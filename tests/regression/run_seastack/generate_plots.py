...
def find_ref(model: str, test_type: str) -> str | None:
	...
	# Prefer new run_seastack expected dir
	preferred = THIS / model / test_type / "expected" / f"ss_ref_{model}_{test_type}.txt"
	if preferred.exists():
...
def main() -> int:
	...
	parser = argparse.ArgumentParser(description="SEA-Stack – regenerate comparison plots from existing outputs")
	...
	parser.add_argument("--f3of", action="store_true", help="Generate plots for all F3OF decay tests (decay_dt1, decay_dt2, decay_dt3)")
	...
	parser.add_argument("--f3of-decay-c1", action="store_true", help="Generate plots for F3OF decay_dt1")
	parser.add_argument("--f3of-decay-c2", action="store_true", help="Generate plots for F3OF decay_dt2")
	parser.add_argument("--f3of-decay-c3", action="store_true", help="Generate plots for F3OF decay_dt3")
	...
	if args.all:
		...
		if args.f3of_decay_c3:
			selections.append(("f3of", "decay_dt3"))
		if not selections:
			print("No selections. Use --all or one of --sphere/--oswec/--rm3/--f3of or --f3of-decay-c1/--f3of-decay-c2/--f3of-decay-c3", file=sys.stderr)
			return 2
