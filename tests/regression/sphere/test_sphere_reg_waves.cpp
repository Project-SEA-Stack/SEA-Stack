...
int main(int argc, char* argv[]) {
    ...
    // Parse command-line arguments for per-condition execution
    int requested_condition = -1;
    if (argc >= 2) {
        std::string arg = argv[1];
        if (arg == "--condition" && argc >= 3) {
            requested_condition = std::atoi(argv[2]);
            if (requested_condition < 1 || requested_condition > reg_wave_num_max) {
                std::cerr << "Error: Condition number must be between 1 and " << reg_wave_num_max << std::endl;
                return 1;
            }
        }
    }

#ifdef REGRESSION_CONDITION_SUBSET
    const std::set<int> condition_subset = { REGRESSION_CONDITION_SUBSET };
#else
    const std::set<int> condition_subset = {};
#endif
    ...
    for (int reg_wave_num = 1; reg_wave_num <= reg_wave_num_max; ++reg_wave_num) {
        // If a specific condition was requested, skip others
        if (requested_condition > 0 && reg_wave_num != requested_condition) {
            continue;
        }
        
        if (!condition_subset.empty() && condition_subset.find(reg_wave_num) == condition_subset.end()) {
            std::cout << "Skipping wave " << reg_wave_num << " (not in core subset)" << std::endl;
            continue;
        }
        ...
        double simulation_duration = seastack::chrono::GetSimDuration(240.0, 800.0);
        ...
        sea_state.amplitude = task10_wave_amps[reg_wave_num - 1];
        sea_state.omega = task10_wave_omegas[reg_wave_num - 1];
        ...
        if (outputFile.is_open()) {
            ...
            outputFile << "Wave amplitude (m): \t" << sea_state.amplitude << "\n";
            outputFile << "Wave omega (rad/s): \t" << sea_state.omega << "\n";
