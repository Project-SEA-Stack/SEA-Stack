...
std::array<double, 3> rotate_vector_3d(std::array<double, 3> vector,
    ...
    for (int i = 0; i < 3; i++) {
        rotated[i] = vector[i] * cos(angle_in_radians) +
...
int main(int argc, char* argv[]) {
    std::cout << "=== OSWEC DECAY WECSIM VERIFICATION ===" << std::endl;
    ...
    auto h5fname =
        (DATADIR / "demos" / "oswec" / "hydroData" / "oswec.h5").lexically_normal().generic_string();

    if (!std::filesystem::exists(body1_meshfname)) {
        std::cerr << "ERROR: flap mesh not found at " << body1_meshfname << std::endl;
        return 1;
    }
    if (!std::filesystem::exists(h5fname)) {
        std::cerr << "ERROR: oswec.h5 not found at " << h5fname << std::endl;
        return 1;
    }
    ...
    std::shared_ptr<ChBody> base_body = chrono_types::make_shared<ChBodyEasyMesh>(
        body2_meshfname, 1000, false, true, false);
    ...
    HydroSystem hydro_forces(bodies, h5fname, no_waves);
    ...
    if (outputFile.is_open()) {
        outputFile << std::left << std::setw(10) << "Time (s)"
                   << std::right << std::setw(16) << "Pitch (rad)" << std::endl;
