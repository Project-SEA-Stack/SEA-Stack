...
void MoorDynWrapper::Initialize(const SystemState& initial_state) {
    ...
    // MoorDyn constructor always emits MSG-level banner to stdout; SEA-Stack
    // stream capture downgrades that when not in debug mode. This call
    // suppresses further MSG/DBG chatter during Init/Step without patching
    // vendored MoorDyn sources.
    if (!seastack::infra::debug::IsDebugEnabled()) {
        (void)MoorDyn_SetVerbosity(system_, MOORDYN_ERR_LEVEL);
    }
    ...
    if (n_coupled_dof_ != expected_dof) {
        throw std::runtime_error(
            ...
            " coupled DOFs but SEA-Stack expected " +
...
void MoorDynWrapper::Step(const SystemState& state, double time, double dt,
    ...
    if (!initialized_) {
        throw std::logic_error("MoorDynWrapper::Step called before Initialize");
    }
    ...
    SEASTACK_TRACE_ONCE(
        "MoorDynWrapper::Step entering PackState at t=" +
        std::to_string(time) + ", dt=" + std::to_string(dt));
    PackState(state, x_, xd_);
    SEASTACK_TRACE_ONCE(
        "MoorDynWrapper::Step returned from PackState at t=" +
        std::to_string(time));

    double md_dt = dt;
    SEASTACK_TRACE_ONCE(
        "MoorDynWrapper::Step entering MoorDyn_Step at t=" +
        std::to_string(time) + ", dt=" + std::to_string(md_dt));
    int err = MoorDyn_Step(system_, x_.data(), xd_.data(), f_.data(),
                           &moordyn_time_, &md_dt);
    SEASTACK_TRACE_ONCE(
        "MoorDynWrapper::Step returned from MoorDyn_Step at t=" +
        std::to_string(time) + ", dt=" + std::to_string(md_dt));
    if (err != 0) {
        throw std::runtime_error(
    ...
    for (size_t ci = 0; ci < coupled_body_indices_.size(); ++ci) {
        ...
        if (bi < 0 || bi >= static_cast<int>(out_forces.size())) {
            throw std::out_of_range(
                "MoorDynWrapper::Step: coupled body index " +
                std::to_string(bi) + " out of range [0, " +
                std::to_string(out_forces.size()) + ")");
        }

        const size_t offset = ci * kDofPerBody;
        for (int d = 0; d < 3; ++d) {
            out_forces[bi].force[d]  += f_[offset + d];
            out_forces[bi].moment[d] += f_[offset + 3 + d];
        }
    }
}

std::vector<seastack::viz::MooringLineVizData> MoorDynWrapper::GetLineStates() const {
    using seastack::viz::MooringPointType;

    if (!initialized_ || !system_) return {};

    SEASTACK_TRACE_ONCE(
        "MoorDynWrapper::GetLineStates first MoorDyn line-state access");
    ...
    std::vector<seastack::viz::MooringLineVizData> result(num_lines);
...
void MoorDynWrapper::PackState(const SystemState& state,
    ...
    for (size_t ci = 0; ci < coupled_body_indices_.size(); ++ci) {
        const int bi = coupled_body_indices_[ci];
        if (bi < 0 || bi >= static_cast<int>(state.bodies.size())) {
            throw std::out_of_range(
                "MoorDynWrapper::PackState: coupled body index " +
                std::to_string(bi) + " out of range [0, " +
