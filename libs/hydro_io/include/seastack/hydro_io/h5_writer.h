 ...
 * datasets required by SEA-Stack exports.
 *
 ...
 *       (or optionally native float via WriteDatasetF32).
 ...
 *********************************************************************/
#ifndef SEASTACK_HYDRO_IO_H5_WRITER_H
#define SEASTACK_HYDRO_IO_H5_WRITER_H
...
/// Options controlling HDF5 dataset storage layout.
struct WriteOptions {
    /// gzip compression level: 0 = none (contiguous), 1-9 = deflate level.
    /// Level 1 is recommended for time-series data (good ratio, low CPU).
  ...
  public:
    ...
    class Group {
      public:
        ...
        // ── Compressed / typed overloads ────────────────────────────────

        /** @brief Write a 1D double dataset with optional gzip compression. */
        void WriteDataset(const std::string& name, const std::vector<double>& data,
                          const std::array<hsize_t, 1>& dims, const WriteOptions& opts) const;
        /** @brief Write a 2D double dataset with optional gzip compression. */
        void WriteDataset(const std::string& name, const std::vector<double>& data,
                          const std::array<hsize_t, 2>& dims, const WriteOptions& opts) const;

        /** @brief Write a 1D dataset stored as float32 (source data is double). */
        void WriteDatasetF32(const std::string& name, const std::vector<double>& data,
                             const std::array<hsize_t, 1>& dims, const WriteOptions& opts) const;
        /** @brief Write a 2D dataset stored as float32 (source data is double). */
        void WriteDatasetF32(const std::string& name, const std::vector<double>& data,
                             const std::array<hsize_t, 2>& dims, const WriteOptions& opts) const;
...
#endif // SEASTACK_HYDRO_IO_H5_WRITER_H
