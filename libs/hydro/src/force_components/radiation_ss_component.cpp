...
constexpr double kKernelZeroThreshold = 1e-10;
constexpr double kFallbackDt = 0.001;
...
RadiationStateSpaceComponent::RadiationStateSpaceComponent(
    ...
    velocity_buffer_.resize(num_dofs_);
    force_radiation_buffer_.resize(num_dofs_);
...
void RadiationStateSpaceComponent::InitializeFromRirf(const HydroData& file_info) {
    ...
    for (int i = 0; i < num_dofs_; ++i) {
        for (int j = 0; j < num_dofs_; ++j) {
            ...
            if (K_norm < kKernelZeroThreshold) {
                ++skipped_count;
                continue;
            }
...
void RadiationStateSpaceComponent::Compute(
    ...
    ExtractVelocityVector(state, velocity_buffer_);
    const Eigen::VectorXd& v = velocity_buffer_;

    // Compute time step
    double dt;
    if (!has_prev_time_) {
        // First call - use a small default dt
        dt = kFallbackDt;  // Will be overwritten next step
        has_prev_time_ = true;
    } else {
        dt = time - prev_time_;
    }
    ...
    // Accumulate radiation forces (use member buffer)
    force_radiation_buffer_.setZero();
    ...
    for (auto& dof_model : dof_pair_models_) {
        ...
        force_radiation_buffer_(dof_model.i) += dof_model.model.GetForce();
    }

    // Add forces to output (with negative sign for damping)
    AccumulateForces(force_radiation_buffer_, inout_forces);
}
...
void RadiationStateSpaceComponent::ExtractVelocityVector(
    const SystemState& state,
    Eigen::VectorXd& out_velocity) const {
    ...
    for (int b = 0; b < num_bodies_; ++b) {
        ...
        out_velocity(offset + 0) = body_state.linear_velocity[0];
        out_velocity(offset + 1) = body_state.linear_velocity[1];
        out_velocity(offset + 2) = body_state.linear_velocity[2];
        
        // Angular velocities (roll, pitch, yaw)
        out_velocity(offset + 3) = body_state.angular_velocity[0];
        out_velocity(offset + 4) = body_state.angular_velocity[1];
        out_velocity(offset + 5) = body_state.angular_velocity[2];
    }
}
...
void RadiationStateSpaceComponent::AccumulateForces(
    ...
    for (int b = 0; b < num_bodies_; ++b) {
        const int offset = b * kDofPerBody;
        for (int d = 0; d < 3; ++d) {
            inout_forces[b].force[d]  -= f_rad(offset + d);
            inout_forces[b].moment[d] -= f_rad(offset + 3 + d);
        }
    }
}
