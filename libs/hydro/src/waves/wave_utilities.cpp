 ...
 *   - Pierson-Moskowitz: Pierson & Moskowitz (1964), "A proposed spectral
 *     form for fully developed wind seas," J. Geophys. Res., 69(24).
...
bool is_in_deep_water(double wavenumber, double water_depth) {
    using wave_detail::kEffectiveInfiniteDepth;
    if (water_depth <= 0.0 || water_depth > kEffectiveInfiniteDepth || std::isinf(water_depth)) {
        return true;
    }
...
Eigen::Vector3d GetWaterVelocity(const Eigen::Vector3d& position,
    ...
    if (is_in_deep_water(wavenumber, water_depth)) {
        ...
        if (std::abs(kh) < wave_detail::kShallowWaterKhThreshold) {
