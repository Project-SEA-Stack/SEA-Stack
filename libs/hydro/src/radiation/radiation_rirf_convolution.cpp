 ...
 *
 * Implements the Cummins equation radiation force term:
 *   F_rad(t) = -A_inf * x_ddot(t) - integral_0^t K(t-tau) * x_dot(tau) dtau
 * where K is the retardation (impulse response) function.
 * See: Cummins, W.E. (1962) "The Impulse Response Function and Ship Motions",
 *      Schiffstechnik, 9, 101-109.
 *********************************************************************/

#include <seastack/hydro/radiation/radiation_rirf_convolution.h>
...
RadiationRirfConvolution::RadiationRirfConvolution(
    ...
    // Pre-allocate force output buffer
    force_radiation_damping_buffer_.resize(total_dofs_);
}
...
void RadiationRirfConvolution::RecordVelocities(
    ...
    // Record current time at the front (most recent first) - O(1) with deque
    time_history_.push_front(simulation_time);

    // Record current velocities per body at the front (matching time_history_ ordering) - O(1) with deque
    for (int b = 0; b < num_bodies_; ++b) {
        auto& velocity_history_body = velocity_history_[b];
        velocity_history_body.push_front(body_velocities[b]);
    }
...
void RadiationRirfConvolution::InterpolateVelocity6D(
    ...
    double older_time,
    double newer_time,
    std::array<double, kDofPerBody>& out_velocity) {
...
std::vector<double> RadiationRirfConvolution::ComputeForces(
    ...
    std::fill(force_radiation_damping_buffer_.begin(), force_radiation_damping_buffer_.end(), 0.0);

    // Nothing to convolve with if we don't yet have at least 2 time points
    if (time_history_.size() <= 1) {
        return force_radiation_damping_buffer_;
    }
    ...
    #pragma omp parallel
    {
        ...
        for (int step = 0; step < rirf_steps_; ++step) {
            ...
            for (int body_index = 0; body_index < num_bodies_; ++body_index) {
                ...
                std::array<double, kDofPerBody> interpolated_velocity_dof{};
    ...
    for (int t = 0; t < num_threads; ++t) {
        ...
        for (int row = 0; row < total_dofs_; ++row) {
            force_radiation_damping_buffer_[row] += local[row];
        }
    }
#else
    for (int step = 0; step < rirf_steps_; ++step) {
        ...
        for (int body_index = 0; body_index < num_bodies_; ++body_index) {
            ...
            for (int dof = 0; dof < kDofPerBody; ++dof) {
                ...
                for (int row = 0; row < total_dofs_; ++row) {
                    ...
                    force_radiation_damping_buffer_[row] += rirf_val * contribution_scale;
                }
            }
        }
    }
    ...
    // Return a copy to avoid exposing the member buffer (caller may hold reference)
    return force_radiation_damping_buffer_;
}
