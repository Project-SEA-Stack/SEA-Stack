    ...
    for (int b = 0; b < num_bodies; ++b) {
        ...
        for (int row_dof = 0; row_dof < rows; ++row_dof) {
            for (int col = 0; col < cols; ++col) {
                ...
                // over [taper_start_fraction, taper_end_fraction] of the RIRF length,
                // then zeros beyond taper_end_fraction.
                if (do_taper) {
                    const int tc_start = std::clamp(
                        static_cast<int>(std::floor(options.taper_start_fraction * rirf_steps)),
                        0, rirf_steps);
                    const int tc_end = std::clamp(
                        static_cast<int>(std::floor(options.taper_end_fraction * rirf_steps)),
                        tc_start, rirf_steps);
        ...
        LOG_DEBUG("RIRF kernel processing (body " << b
                  ...
                  << (do_taper ? (", Start: " + std::to_string(options.taper_start_fraction)
                                + ", End: " + std::to_string(options.taper_end_fraction)
        ...
        if (options.export_csv) {
            try {
                ...
                if (b == 0) {
                    LOG_DEBUG("RIRF CSVs written in " << out_dir.string());
                }
