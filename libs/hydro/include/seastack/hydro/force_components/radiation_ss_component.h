...
#ifndef SEASTACK_HYDRO_FORCE_COMPONENTS_RADIATION_SS_COMPONENT_H
#define SEASTACK_HYDRO_FORCE_COMPONENTS_RADIATION_SS_COMPONENT_H

#include <seastack/core/force_component.h>
#include <seastack/core/system_state.h>
#include <seastack/hydro/radiation_types.h>
#include <seastack/hydro/radiation/radiation_ss_model.h>
#include <seastack/hydro/radiation/radiation_ss_fitter.h>
#include <Eigen/Dense>
#include <vector>
#include <memory>
...
public:
     ...
     * @return HydroComponentType::kRadiation
     */
    HydroComponentType Type() const override { return HydroComponentType::kRadiation; }
...
private:
    ...
    // Reusable buffers to avoid per-timestep allocations
    Eigen::VectorXd velocity_buffer_;      ///< Buffer for velocity vector extraction
    Eigen::VectorXd force_radiation_buffer_;  ///< Buffer for radiation force accumulation
     ...
     * @param out_velocity Output vector [6*num_bodies] (linear, angular for each body)
     */
    void ExtractVelocityVector(const SystemState& state, Eigen::VectorXd& out_velocity) const;
...
#endif  // SEASTACK_HYDRO_FORCE_COMPONENTS_RADIATION_SS_COMPONENT_H
