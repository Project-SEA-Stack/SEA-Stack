...
// Version information - defined by the build system via config.h/version.h
...
void LoggerBackend::Log(LogLevel level, const std::string& message, 
    ...
    // Write to console: full CLI when enabled; in quiet mode (CLI off) still show
    // warnings and errors so --quiet remains usable for automation.
    const bool console_on =
        (config_.enable_cli_output && ShouldLog(level, true)) ||
        (!config_.enable_cli_output &&
         static_cast<int>(level) >= static_cast<int>(LogLevel::Warning));
    if (console_on) {
...
std::string LoggerBackend::CreateLogHeader() {
    ...
    ss << "============================================================\n";
    ss << " SEA-Stack Simulation Log\n";
    ss << "============================================================\n";
    ss << " Executable:          " << executable_name_ << "\n";
    ss << " SEA-Stack version:   " << SEASTACK_VERSION << "\n";
    ss << " Build type:          " << SEASTACK_BUILD_TYPE << "\n";
    ss << " Platform:            ";
#ifdef _WIN32
    ss << "Windows";
#elif defined(__APPLE__)
...
//-----------------------------------------------------------------------------
// GetExecutableDirectory (for exe-relative data path resolution)
//-----------------------------------------------------------------------------
std::string GetExecutableDirectory() {
    ...
    if (p.has_parent_path()) {
        return p.parent_path().lexically_normal().generic_string();
    }
    return "";
}
...
std::string GetPlatformSummary() {
    ...
    if (GlobalMemoryStatusEx(&mem)) {
        result += ", " + std::to_string(mem.ullTotalPhys / (1024*1024*1024)) + " GB RAM";
    }
#elif defined(__APPLE__)
    result = "macOS";
#else
