"""Generate the support-vessel Wigley hull mesh for the ROV umbilical demo.

A single 50 m Wigley hull, identical in form to the wigley and ship2ship demos.
The parametrisation is imported from the wigley demo assets rather than
duplicated, so the demos cannot drift apart.
...
try:
    ...
    raise ImportError(
        ...
        "The rov demo reuses data/demos/run_seastack/wigley/assets/generate_meshes.py."
    ) from exc
...
def generate(output_dir: Path) -> None:
    ...
    nemoh_path = meshes_dir / "vessel.nemoh"
    write_nemoh(nemoh_path, vertices, quads)
    print(f"  Nemoh: {nemoh_path.name}  ({len(vertices)} verts, {len(quads)} panels)")

    obj_path = geom_dir / "vessel.obj"
    write_obj(obj_path, vertices, quads, header="ROV support vessel, Wigley hull (50 m)")
...
def main() -> None:
    ...
    print("Generating ROV support vessel mesh...\n")
