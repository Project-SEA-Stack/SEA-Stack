// 5SA with bimodal sea state (swell + wind sea) + MoorDyn mooring.
// Demonstrates multi-partition directional wave field with different
// heading and spreading for each sea-state component.
...
int main(int argc, char* argv[]) {
    ...
    std::string data_dir;
    if (!seastack::chrono::GetCLIArguments(argc, argv, "5SA bimodal demo",
    ...
    // Bimodal sea state: swell from 0 deg + wind sea from 90 deg
    SeaStateDefinition sea_state;
    sea_state.type = "irregular";
    sea_state.depth = 50.0;

    SeaStatePartition swell;
    swell.spectrum.type = "jonswap";
    swell.spectrum.Hs = 2.0;
    swell.spectrum.Tp = 12.0;
    swell.spectrum.gamma = 2.5;
    swell.spreading.type = "cos2s";
    swell.spreading.mean_direction_deg = 0.0;
    swell.spreading.s = 25.0;

    SeaStatePartition wind_sea;
    wind_sea.spectrum.type = "jonswap";
    wind_sea.spectrum.Hs = 1.2;
    wind_sea.spectrum.Tp = 6.0;
    wind_sea.spectrum.gamma = 3.3;
    wind_sea.spreading.type = "cos2s";
    wind_sea.spreading.mean_direction_deg = 90.0;
    wind_sea.spreading.s = 8.0;

    sea_state.partitions.push_back(swell);
    sea_state.partitions.push_back(wind_sea);
    ...
    ui.Init(&system, "5SA - Bimodal Sea State");
    ...
    while (system.GetChTime() <= simulationDuration) {
        ...
        if (ui.simulationStarted) {
            ...
            body3_sway.push_back(five_sa.bodies[2]->GetPos().y());
        }
    }
    ...
    if (saveDataOn) {
        ...
        const std::string output_path = out_dir + "/5sa_bimodal.txt";
        ...
        if (!outputFile.is_open()) {
            ...
            outputFile << std::left << std::setw(10) << "Time (s)"
                       << std::right << std::setw(16) << "Body3 Heave (m)"
                       << std::right << std::setw(16) << "Body3 Sway (m)" << std::endl;
            for (size_t i = 0; i < time_vector.size(); ++i)
                outputFile << std::left << std::setw(12) << std::setprecision(6) << std::fixed << time_vector[i]
                           ...
                           << std::right << std::setw(16) << std::setprecision(6) << std::fixed << body3_sway[i]
                           << std::endl;
        }
    }
