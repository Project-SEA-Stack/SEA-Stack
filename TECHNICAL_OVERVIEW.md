...
This architecture is designed to enable multi-fidelity modelling: the same system models and domain logic can be reused across different simulation backends (e.g. potential flow, CFD, SPH). While v1.0.0-beta.3 uses linear potential-flow hydrodynamics, the structure supports higher-fidelity methods without rewriting the system model.
...
SEA-Stack v1.0.0-beta.3 includes:
...
## 3. Scope of SEA-Stack v1.0.0-beta.3
...
SEA-Stack v1.0.0-beta.3 separates domain physics from the dynamics engine: hydro as a reusable library, PTO/control as light modules, Chrono as the current backend with a thin adapter. That layout supports standalone use, alternative solvers in principle, and future fidelity options.
